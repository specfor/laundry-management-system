...
import { PaginationAdapter } from "../../types";

export function useCustomPaginationAdapter<T>(
    recordFetcher: (currentPage: number, currentPageSize: number, date?: Date, narration?: string) => Promise<T[]>
) {
    return async function paginator<T>(transformer: T) {
        const records = ref((await recordFetcher().catch(() => [])).map(record => ({ ...record, selected: false }))) as Ref<InternalRow[]>;
